  ...
  return (
    <DashboardLayout>
      <div className="space-y-8 max-w-7xl mx-auto">
        {/* Header */}
        <div>
          <h1 className="text-4xl font-extrabold tracking-tight">Analytics Hub</h1>
          <p className="text-muted-foreground mt-2 text-base">Connect your platforms, analyze sales, and get AI-powered business insights.</p>
        </div>

        {/* ─── Platform Connections ─── */}
        <Card className="bg-card shadow-sm border border-border/60 rounded-2xl p-8">
          <h2 className="text-lg font-semibold mb-5">Platform Connections</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {PLATFORMS.map((p) => {
              const connected = isConnected(p.id);
              const conn = connections.find((c) => c.platform === p.id);
              return (
                <div key={p.id} className={`flex items-center gap-4 p-5 rounded-xl border transition-all ${connected ? "border-primary/30 bg-primary/5" : "border-border/50 bg-muted/30"}`}>
                  <div className="w-12 h-12 rounded-xl bg-background border border-border/50 flex items-center justify-center shrink-0 p-2">
                    <img src={p.logo} alt={p.name} className="w-8 h-8 object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                  </div>
                  ...
                  <div className="shrink-0">
                    {connected ? (
                      <Button variant="outline" size="sm" onClick={() => handleDisconnect(p.id)} className="rounded-xl h-10 px-4">
                        <Unlink className="w-4 h-4 mr-1.5" /> Disconnect
                      </Button>
                    ) : (
                      <Button size="sm" onClick={() => setConnectModal(p.id)} className="rounded-xl h-10 px-5">
        ...
        <Card className="bg-card shadow-sm border border-border/60 rounded-2xl p-8">
          <div className="flex items-center justify-between mb-5">
            ...
            <div className="flex items-center gap-2">
              ...
              <Button variant="outline" size="sm" onClick={fetchAnalyticsData} disabled={loadingData || connections.length === 0} className="rounded-xl h-10 px-4">
                <RefreshCw className={`w-4 h-4 mr-1.5 ${loadingData ? "animate-spin" : ""}`} /> Refresh
              </Button>
            </div>
          </div>
          ...
          {connections.length === 0 && !loadingConnections ? (
            <div className="py-16 text-center">
              ...
              <p className="text-muted-foreground text-sm">Connect Whop or Payhip above to start tracking your sales data.</p>
            </div>
          ) : (
            <>
              {/* Metric Cards */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {[
                  ...
                  <div key={m.label} className="p-5 rounded-xl border border-border/50 bg-muted/20">
                    {loadingData ? (
              ...
              {salesChartData.length > 0 && (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                  ...
                  <div className="rounded-xl border border-border/50 p-5">
                    <h3 className="text-sm font-semibold mb-3">Sales Volume</h3>
                    <div className="h-64">
                      <ResponsiveContainer width="100%" height="100%">
        ...
        <Card className="bg-card shadow-sm border border-border/60 rounded-2xl overflow-hidden">
          <div className="p-6 pb-4 border-b border-border/40">
            ...
            <p className="text-sm text-muted-foreground mt-1">Ask questions about your sales data and get personalized business advice.</p>
          </div>
            ...
            <div className="p-5 space-y-4">
              {chatMessages.length === 0 && (
                <div className="text-center py-12">
                  ...
                  <p className="text-muted-foreground mb-4">Ask me anything about your sales performance, marketing strategies, or business growth!</p>
                  <div className="flex flex-wrap justify-center gap-2">
                    {["How are my sales trending?", "Which product performs best?", "How can I increase conversions?"].map((q) => (
                      <Button key={q} variant="outline" size="sm" onClick={() => setChatInput(q)} className="rounded-full text-xs h-8 px-4">{q}</Button>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div className="border-t border-border/40 p-4">
            <form onSubmit={(e) => { e.preventDefault(); handleSendChat(); }} className="flex gap-2">
              <Input
                ...
                className="flex-1 h-12 rounded-xl border-border/60 focus-visible:ring-primary/40 text-base"
              />
              <Button type="submit" disabled={chatLoading || !chatInput.trim()} className="h-12 w-12 rounded-xl p-0">
      ...
      <Dialog open={!!connectModal} onOpenChange={() => { setConnectModal(null); setApiKeyInput(""); }}>
        <DialogContent>
          ...
          <div className="space-y-4 pt-2">
            ...
            <Button onClick={handleConnect} disabled={connecting || !apiKeyInput.trim()} className="w-full h-12 rounded-xl">
