...
import {
  ...
  CheckCircle2, Image as ImageIcon, RefreshCw,
} from "lucide-react";
...
const EbookGenerator = () => {
  ...
  const startGeneration = async () => {
    ...
    try {
      ...
      // STEP 1 — Generate content via new generate-ebook function
      // New response format: { title, chapters: [{ title, content }], frameworks, chapterCount, language }
      const { data: genData, error: genError } = await supabase.functions.invoke("generate-ebook", {
        body: { topic, description, category, targetAudience, tone, language: "English" },
      });
      if (genError) throw new Error(genError.message);
      if (!genData?.title || !genData?.chapters) throw new Error(genData?.error || "Content generation failed");

      // STEP 2 — Cover image via cover-image function
      setStep("cover");
      let coverBase64: string | null = null;
      try {
        const { data: coverData } = await supabase.functions.invoke("cover-image", {
          body: { topic, title: genData.title },
        });
      ...
      // Draw cover
      const subtitle = genData.chapters[0]?.content?.substring(0, 120) || "";
      await renderer.drawCover(genData.title, subtitle, topic, coverBase64);
      renderer.drawTitlePage(genData.title);

      // Build TOC from chapters array
      const toc = genData.chapters.map((ch: any, i: number) => ({
        type: "chapter",
        number: i + 1,
        label: ch.title,
      }));
      renderer.drawTOC(toc, genData.title);

      // Draw each chapter with correct incrementing number
      for (let i = 0; i < genData.chapters.length; i++) {
        const ch = genData.chapters[i];
        renderer.drawChapterSplash(i + 1, ch.title, genData.title);
        renderer.drawContentPages(ch.title, ch.content, genData.title, true, i + 1);
      }

      const safeTitle = genData.title.replace(/[^a-z0-9]/gi, "_").toLowerCase();
      const filename  = `${safeTitle}.pdf`;

      // STEP 4 — Build ebook object
      const flatContent = [
        `# ${genData.title}`,
        ...genData.chapters.map((c: any) => `## ${c.title}\n\n${c.content}`),
      ].join("\n\n");
      ...
      // Build chapters list for outline screen
      setChapters(
        genData.chapters.map((ch: any, i: number) => ({
          id:          i + 1,
          title:       ch.title,
          description: `Chapter ${i + 1} of your ebook`,
          phase:       `Phase ${Math.ceil((i + 1) / 3)}`,
          modules:     `${((i + 1) % 4) + 2} Sections`,
        }))
      );
  ...
  const resetForm = () => {
    ...
    setTone("professional"); setErrorMsg(null);
  };
